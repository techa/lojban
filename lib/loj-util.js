...
module.exports = class LojbanUtil {
  /**
   * [constructor description]
   * @param  {Object} parser_options  - 例：{unusedLetter:true}
   * @return {this}
   */
  constructor(parser_options) {
    // PARSERのコピーを作成
    let parsers = Object.assign({}, PARSER);
    // parsersにオプションを反映
    if (parser_options) {
      Object.keys(parser_options).forEach( key => {
        if (parsers[key]) {
          parsers[key][0] = !!parser_options[key];
        }
      });
    }
