...
DATA.valsis.forEach((valsi) => {
  ...
  if (valsi.selmaho) {
    ...
    ;[
      ...
      wordData[KEY_NAMES.SELMAHO].push(parser[0].test(valsi.selmaho) ? parser[1] : '')
    })

    const match = /^(\w+)(\*)?(\d)?$/.exec(valsi.selmaho)
    if (match[1]) {
      wordData[KEY_NAMES.SELMAHO].push(match[1])
    }
    if (+match[3]) {
      wordData[KEY_NAMES.SELMAHO].push(+match[3])
    }
    if (match[2]) {
      wordData[KEY_NAMES.SELMAHO].push(!!match[2])
    }
  }
