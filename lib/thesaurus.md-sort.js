...
let dictionary = JSON.parse(fs.readFileSync(DICTIONARY_FILE_PATH, 'utf8'))

const originText = fs.readFileSync(readFilePath, 'utf8')
const lines = originText.split(/[\r?\n]/).filter((lines) => lines) // ????
let expText = ''
...
if (/addnum/i.test(argv[0])) {
  // add number
  expText += lines.map((line, i) => `|${i + 1}` + line).join('\n')
  ...
  if (argv[1] === 'num') {
    ...
    lines.some((line) => {
      ...
      // http://graphemica.com/search?q=ʌɪɔəɴħŋɛ
    ...
    expText += lines
      .map((line, i) => {
        return line.slice(0, 9) + parser(line.slice(9, 14)) + line.slice(14)
      })
      .join('\n')
  }
} else {
  ...
  switch (col) {
    ...
    case 4:
      ...
      expText += lines
        .map((line, i) => {
          // console.log(i + 1, parseInt(line.slice(1, 6)) + j)
          if (i + 1 !== parseInt(line.slice(1, 6))) {
    ...
    case 5:
      ...
      expText += lines
        .map((line, i) => {
          return '| ' + (i + 1 + '').padStart(4, ' ') + line.slice(6)
        })
        .join('\n')
      break
    ...
    case 0:
      for (let key in dictionary) {
        if (dictionary.hasOwnProperty(key)) {
          const valsi = dictionary[key]
          if (valsi.type === 'gismu') {
            if (!lines.some((line) => line.slice(9, 14) === valsi.word)) {
              console.log('valsi.word', valsi.word, valsi.unofficial)
            }
          }
        }
      }
